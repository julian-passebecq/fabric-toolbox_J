...
def _safe_bundle_name(value: str) -> str:
    ...
    return safe or "fabric-project"


def build_vscode_handoff_files(
    template: ProjectTemplate,
    request: ProjectManifestExportRequest,
) -> dict[str, Any]:
    """Build a portable VS Code folder handoff without inventing an unsupported Fabric deep link."""
    manifest = export_project_manifest(template, request)
    environment = str(manifest.parameters.get("environment") or "project")
    bundle_name = _safe_bundle_name(f"{template.id}-{environment}")
    ...
    authoring_items = [
        {
            ...
            "definitionStrategy": item.definition_strategy,
            "dependsOn": item.depends_on,
        }
        for item in template.items
        if item.vscode_handoff
    ]
    ...
    handoff = {
        ...
        "authoring_items": authoring_items,
        "operator_steps": [
            "Open the .code-workspace file in VS Code.",
            "Install the recommended Microsoft Fabric and Fabric MCP Server extensions.",
            "Run Fabric: Sign in from the Command Palette.",
            "In Fabric Explorer, select the workspace named in fabric-handoff.json.",
            "Use VFS mode to edit remote item definitions directly, or clone the workspace when Fabric Git integration is enabled.",
            "Use the generated fabric-project.json as the desired-state reference when editing definitions.",
        ],
        "deep_link": None,
        "deep_link_note": (
            "No workspace-by-ID VS Code URI is emitted because Microsoft does not document a stable "
            "Fabric extension deep-link contract for arbitrary workspaces."
        ),
    }
    ...
    workspace_display = manifest.workspace_name
    workspace_id_text = manifest.workspace_id or "not selected in Studio"
    readme = f"""# {template.name} - VS Code handoff

This folder was generated by Fabric Ops Studio from project template {template.id} v{template.version}.

## Fabric target

- Workspace name: {workspace_display}
- Workspace ID: {workspace_id_text}
- Template SHA-256: {manifest.template_sha256}

## Open the project

1. Open {bundle_name}.code-workspace in Visual Studio Code.
2. Install the recommended Microsoft Fabric and Microsoft Fabric MCP Server extensions.
3. Open the Command Palette and run Fabric: Sign in.
4. In Fabric Explorer, select {workspace_display}.
5. Use Fabric VFS mode to edit supported remote item definitions directly. If the workspace is Git-enabled, you can instead use the Fabric extension clone/version-control workflow.
6. Keep fabric-project.json as the desired-state reference exported from Fabric Ops Studio.

## MCP

.vscode/mcp.json configures the current remote Fabric Core MCP endpoint for VS Code-compatible MCP clients. Authentication is performed by the client with your Microsoft identity and Fabric permissions.

The recommended Fabric MCP Server extension is also included because Microsoft documents it as the local development companion for API schemas, item definitions, OneLake tools and Fabric-aware agent workflows.

## Authoring scope

The intended VS Code authoring items are listed in fabric-handoff.json. Fabric Ops Studio remains the guarded control plane for reviewed provisioning and definition reconciliation.

## Safety

- No secret project values are included.
- No Fabric access tokens or credentials are included.
- No Change Plan confirmation text is included.
- No mutation plan is included or restored.
- This bundle does not auto-execute Fabric changes.
- No undocumented workspace deep link is generated.

## Current Microsoft workflow

The Microsoft Fabric extension supports sign-in, workspace/item browsing, editing supported item definitions, Git-enabled workspace cloning/version control, and VFS workspace access. Select the target workspace from Fabric Explorer after sign-in.
"""

    files = {
        f"{bundle_name}.code-workspace": json.dumps(workspace_file, indent=2),
        ".vscode/extensions.json": json.dumps(extensions_file, indent=2),
        ".vscode/mcp.json": json.dumps(mcp_file, indent=2),
        "fabric-project.json": manifest.model_dump_json(indent=2),
        "fabric-handoff.json": json.dumps(handoff, indent=2),
        "README.md": readme,
    }
