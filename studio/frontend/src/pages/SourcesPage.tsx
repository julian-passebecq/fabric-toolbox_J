...
const useStyles = makeStyles({
  ...
  code: { display: 'block', padding: '9px 10px', backgroundColor: tokens.colorNeutralBackground3, borderRadius: tokens.borderRadiusMedium, fontFamily: 'Consolas, monospace', overflowWrap: 'anywhere' },
    ...
    <>
      <div className={styles.header}>
        <div>
          ...
          <Text block>Provider provenance and update strategy for every upstream dependency used by Fabric Ops Studio.</Text>
        </div>
          ...
          <div className={styles.grid}>
            {registry.sources.map((source) => (
              <Card key={source.id}>
                ...
                {source.local_path && <code className={styles.code}>{source.local_path}</code>}
                {source.update_strategy && <Text block>Update: {source.update_strategy}</Text>}
